...
async function writeInBatches(collectionName, records, buildDoc) {
  let batch = writeBatch(db);
  let count = 0;
  for (const record of records) {
    batch.set(doc(db, collectionName, buildDoc(record).id), buildDoc(record));
    count += 1;
    if (count >= 400) {
      await batch.commit();
      batch = writeBatch(db);
...
async function main() {
  try {
    console.log("\n🚀 Writing core Firestore data (stations, chargers, slots)\n");

    console.log("  📍 Stations...");
    await writeInBatches("stations", STATIONS, (s) => ({
      id: s.id,
      name: s.name,
      address: s.address,
      latitude: s.latitude,
    ...
    await writeInBatches("chargers", CHARGERS, (c) => c);
    console.log(`     ✓ ${CHARGERS.length} chargers written`);

    console.log("  ⏰ Slots (24h, 30-min intervals)...");
    ...
    for (let minutes = 0; minutes < 24 * 60; minutes += 30) {
      ...
      for (const station of STATIONS) {
        const id = `${dateStr}-${station.id}-${startTime.replace(":", "")}-${endTime.replace(":", "")}`;
        slots.push({
          id,
    ...
    await writeInBatches("slots", slots, (slot) => slot);
    console.log(`     ✓ ${slots.length} slots written`);

    console.log("\n✅ Core data restore complete\n");
    console.log(`   Project: ${firebaseConfig.projectId}`);
    console.log("   Collections written: stations, chargers, slots");
  } catch (error) {
